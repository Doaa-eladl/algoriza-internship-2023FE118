<template>
  <div class="flex mx-auto w-[1240px] items-center my-4" :class="[ componentname=='login' ? 'justify-start' : 'justify-between']">
    ...
    <div v-if="componentname!='login' && componentname!='booking'">
      <nav>
        <ul class="grid grid-cols-5 gap-x-12">
          ...
          <li class=" text-base	font-normal	">About</li>
          <li class=" text-base	font-normal	">Contact</li>
        </ul>
      </nav>
    </div>
    <div class="flex " v-if="componentname!='login'">
      <router-link :to="{ name: 'register'}" class="bg-light-blue text-white	px-4 py-2.5 text-sm font-medium	leading-5	rounded-md	">Register</router-link>
    </div>
  </div>
</template>
...
export default {
  setup(){
    ...
    let componentname = ref(route.meta.componentname)
